...
public class RopeRoot : MonoBehaviour {
    ...
    private void AddChildren(Transform parent){
        for (int i = 0; i < parent.childCount; i++)
        {
            ...
            //DistanceJoint
            var joint = representative.gameObject.AddComponent<DistanceJoint>();
    ...
    public void Update() {
        for (int i = 0; i < CopySource.Count; i++)
        {
            ...
            CopySource[i].rotation = CopySource[i].GetComponent<DistanceJoint>().ConnectedRigidbody.transform.rotation;
        //     if (i != CopySource.Count - 1) {
        //     Vector3 direction = (CopySource[i].transform.position - CopySource[i+1].transform.position).normalized;
        //         CopySource[i].transform.LookAt(CopySource[i].transform.position + direction);
        //     } else {
        //         CopySource[i].transform.LookAt(transform);
        //     }
        }
    }
}